...
const DynamicInsightCard = ({ title, value, percentage, isUp, bgColor = "bg-blue-50", darkColor}) => {
  ...
  return (
    ...
    >
      {/* Title */}
      {darkColor === "bg-[#272727]" && darkMode ? <h3 className="text-white text-base font-medium">{title}</h3> : <h3 className="text-gray-800 text-sm font-semibold">{title}</h3>}

      <div className="flex items-end justify-between">
        {/* Main Value */}
        {darkColor === "bg-[#272727]" && darkMode ?<span className="text-3xl font-semibold text-white">{value}</span>:<span className="text-2xl font-semibold text-gray-900">{value}</span>}
